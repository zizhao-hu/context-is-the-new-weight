...
context $C$: prepending $C$ at inference time, which we call
\textbf{in-context}; and fine-tuning the model on synthetic $(Q, A)$
pairs where $A$ has been generated under $C$, which we call
\textbf{context-distilled}. Both interventions are evaluated against
the unmodified \textbf{base} model. We do not compare in weight space;
we compare \emph{activations}, layer by layer, on Llama-3.1-8B-Instruct
across five context types --- structure constraint (high and low),
style constraint, few-shot, and factual injection --- with two
complementary measures: (i) how each intervention changes residual-
stream activations relative to base, and (ii) how similar the two
resulting activation perturbations are to each other. We find that
context-distilled produces a residual-stream perturbation aligned with
the in-context perturbation at every layer, with cosine reaching
$0.42$--$0.80$ at peak sites in the last third of the network --- but
with consistently larger magnitude than the in-context perturbation.
The activation pattern is invariant across training and held-out
validation queries, ruling out memorisation. The two interventions
agree on direction but not on amount: they are related, not identical,
...
The headline result of this paper is that in-context and
context-distilled produce \emph{aligned but not identical} residual-
stream perturbations. The cosine reaches $0.42$--$0.80$ in late layers
across all five contexts, but
$\|v^\text{dist}\| > \|v^\text{ctx}\|$ at every layer with the gap
widening with depth. The two interventions are therefore related ---
they push the model in roughly the same direction in activation space
--- but they are not interchangeable. Context-distilled is a louder,
sharper version of in-context, not a faithful reproduction of it. This
matters for any setting where a practitioner treats the two as
equivalent (e.g.\ caching ``a fine-tuned version of this prompt''):
the resulting outputs may be more committed and less hedged than the
