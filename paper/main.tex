...
memorisation. Crucially, evaluating both variants on the unrelated
MMLU-Pro benchmark reveals that in-context preserves general
capability ($\Delta$ acc $\in [-0.07, +0.02]$ across all seven
contexts), while context-distilled destroys it for most contexts
($\Delta$ acc as low as~$-0.24$). The two interventions agree on
direction but not on selectivity: in-context applies the perturbation
\emph{conditionally} via attention; context-distilled applies it
\emph{indiscriminately}, sacrificing capability on out-of-distribution
inputs for permanence on the training distribution. Context-distillation
is therefore not a clean substitute for context compression.
\end{abstract}
