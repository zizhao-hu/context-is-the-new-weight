...
$\mathrm{R}_L(\text{base}, \text{in-context})$ on the same OOD
...
\begin{table}[t]
\centering
\caption{ROUGE-L between model variants' generated answers, on the
in-distribution validation set vs out-of-distribution MMLU-Pro. The
in-context teacher's answers track base on OOD (the model
selectively ignores irrelevant $C$), but the context-distilled
student's answers diverge sharply from the teacher's on OOD --- the
student cannot ignore $C$ when irrelevant.}
\label{tab:rouge_oodvsid}
\small
\begin{tabular}{lcccc}
\toprule
Context & R$_L$(stu, in-ctx) in-dist & R$_L$(stu, in-ctx) OOD & R$_L$(base, in-ctx) in-dist & R$_L$(base, in-ctx) OOD \\
\midrule
haiku                  & 0.523 & 0.301 & 0.226 & 0.301 \\
pirate                 & 0.610 & 0.168 & 0.322 & 0.429 \\
concise                & 0.661 & 0.531 & 0.421 & 0.700 \\
fewshot\_translate\_fr & 0.767 & \textbf{0.006} & 0.459 & 0.600 \\
factual                & 0.515 & 0.120 & 0.364 & 0.331 \\
\bottomrule
\end{tabular}
\end{table}
