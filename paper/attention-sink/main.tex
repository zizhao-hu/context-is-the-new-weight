...
\label{sec:exp}
\label{sec:method}
\paragraph{Setup.} A constant working memory restricts attention to the $W$ most-recent tokens: during finetuning we replace the causal mask $M_{\text{causal}}(t,k)=\mathbb{1}[k\le t]$ with a sliding-window mask $M_{\text{win}}(t,k)=\mathbb{1}[\,t-W<k\le t\,]$ (Fig.~\ref{fig:windowed}), and continue pretraining on generic text (language-modelling loss only, no task supervision) so the comparison isolates the effect of the \emph{mask}. A plain window deletes position~$0$ and a pretrained model collapses; StreamingLLM \citep{xiao2024streamingllm} patches this at inference by retaining a few initial tokens, whereas we remove the dependency by training, the sense in which ``context becomes weight.'' We study three schemes: \textbf{windowed} (the plain sliding window, its cold start filled with real preceding tokens); \textbf{startup} (prepend $P$ trainable soft prompts that fill the window's cold start, a learned replacement for the sink token that slides out of the window for deep tokens, granting deployability from token~1); and \textbf{persistent-prompt} ($P$ trainable prompts \emph{always} attendable by every content token, giving effective memory $W{+}P$ of which $P$ is fixed registers). We evaluate on Qwen2.5-0.5B (a pure-softmax proof of concept) and the hybrid Qwen3.5-9B (the target); finetuning is on WikiText-103 \citep{merity2017wikitext} and the headline window is $W{=}256$. We state and test each hypothesis in turn, each as a claim with its mechanism and results together.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/windowed_attention.png}
\caption{Windowed attention finetuning. The triangular mask (left) is replaced by a sliding window (a band of width $W$), giving each token a constant working memory of the recent $W$ tokens; older context must be carried by the weights. \textbf{Startup} and \textbf{persistent-prompt} prepend trainable prompts that fill the cold start or stay always-attendable, respectively.}
\label{fig:windowed}
\end{figure}
