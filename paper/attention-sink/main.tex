...
\begin{itemize}
\item We \textbf{disentangle two types of attention sink by \emph{position and cause}} --- a \emph{positional} sink on the first token and a content-driven \emph{no-op} sink on low-information tokens (commas, periods) --- orthogonal to the \emph{functional} split (no-op vs.\ broadcast) of \citet{fesser2026unifying}, and trace them to two \emph{different mechanisms}: the positional sink to the triangular mask's exposure imbalance, the no-op sink to content-driven bounded-memory token usage (Sec.~\ref{sec:sink}--\ref{sec:hyp}). We show by finetuning that the positional sink is \emph{learned and removable} --- a windowed mask drives it to zero --- while the no-op sink remains and is leaned on more.
...
\paragraph{Functional vs.\ positional categorization.} Prior work separates sinks by \emph{function}, but not by \emph{position} or by \emph{cause}. \citet{fesser2026unifying} distinguish two algorithms behind a sink --- an adaptive \emph{no-op} that suppresses the update by routing to a null token (negligible value norm) and a \emph{broadcast} that aggregates and redistributes global information (low-rank output) --- while earlier accounts simply retain the initial tokens at inference \citep{xiao2024streamingllm,han2024lminfinite}. We categorize \emph{orthogonally}, by \emph{where} the sink sits: the first token (Type~1) vs.\ low-information separators (Type~2). The two axes cross-cut (Table~\ref{tab:categorization}) --- a sink at either position can serve either function --- so the functional split alone cannot assign a \emph{cause}. The positional split can, and this is our central claim (H1): the two positional types arise from \emph{two different mechanisms}. Type~1 is an artifact of the triangular mask's positional exposure imbalance (H2): a property of the \emph{architecture} that disappears when the mask changes. Type~2 is content-driven, keyed to token \emph{identity}: a property of the \emph{data and the model's use of it} that persists --- indeed is leaned on \emph{more} --- once Type~1 is removed (H3). Separating the sink \emph{by position} is what lets us remove the harmful, mask-induced one while keeping (and exploiting) the useful one. We state these mechanisms as testable hypotheses next.
...
\paragraph{H1: the attention sink is two disentanglable types with distinct causes.} We hypothesize that what is usually called ``the attention sink'' is in fact two phenomena that can be separated. \emph{Type~1} is positional --- a large, content-independent share of attention parked on position~$0$. \emph{Type~2} is content-based --- attention dumped onto semantically near-empty separator tokens (commas, periods, delimiters) used as local no-op registers, keyed to token \emph{identity} rather than to position. Prior work separates sinks by \emph{function} \citep{fesser2026unifying} but not by position or cause; we claim the two \emph{positional} types arise from \emph{two different mechanisms} (H2 and the data, respectively) and can therefore be removed or kept independently.
