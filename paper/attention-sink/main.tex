...
\caption{Four continued-pretraining regimes, each a window of attention (rows = the token being predicted, columns = the positions it attends; a short prefix region plus the content tokens). All three windowed variants hold a constant width $W$; causal grows. \textbf{Top:} the standard \emph{causal} mask, where context grows with position; and a \emph{sliding window} of constant width $W$ whose cold start is filled with real prior text (light green). \textbf{Bottom, our constant-memory variants:} a \emph{uniform window}, where every token is conditioned on exactly $W$ real tokens via fixed-length training (predicting only each window's last token, so it also runs on a recurrence); and a \emph{sliding window with trainable tokens}, where a trainable startup prompt (orange) fills the cold start in place of real history and slides out as content accumulates. Older real context must be carried by the weights.}
\label{fig:windowed}
\end{figure}
