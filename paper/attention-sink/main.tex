...
\paragraph{Method.} To test this we finetune under a sliding-window mask $M_{\text{win}}(t,k)=\mathbb{1}[\,t-W<k\le t\,]$ that replaces the causal mask $M_{\text{causal}}(t,k)=\mathbb{1}[k\le t]$ (Fig.~\ref{fig:windowed}), so each token attends only to the $W$ most-recent positions. We compare the \textbf{base} model, a full-causal continued-pretraining control (\textbf{triangle}), the plain \textbf{windowed} model, and a \textbf{startup} variant that prepends $P$ trainable soft prompts filling the window's cold start, a learned replacement for the sink token that slides out of the window for deep tokens.
