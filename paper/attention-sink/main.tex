...
\caption{Four continued-pretraining regimes (rows = a predicting step, columns = attended positions; token labels are coloured \textcolor[rgb]{0.33,0.33,0.33}{grey} for \emph{context} and \textcolor[rgb]{0.70,0.11,0.11}{crimson} for \emph{predicted}). \textbf{Top:} the standard \emph{causal} mask, where context grows with position; and a \emph{sliding window} of constant width $W$ whose cold start is filled with real prior text (light green). Both predict \emph{every} token. \textbf{Bottom:} a \emph{uniform window} --- the constant-$W$ context shown as a solid block, trained on fixed-length sequences to predict only the \emph{last} token (crimson, the extra column $c_6$), so it also runs on a recurrence; and a \emph{sliding window with trainable tokens}, whose cold start is filled by a trainable startup prompt (orange) that slides out as content accumulates. Older real context must be carried by the weights.}
\label{fig:windowed}
\end{figure}
