...
\begin{itemize}
\item We \textbf{disentangle two types of attention sink by \emph{structure and cause}} --- a \emph{concentrated} sink on the first token and a \emph{sparse} sink on low-information separators (commas, periods) --- orthogonal to the \emph{functional} split (no-op vs.\ broadcast) of \citet{fesser2026unifying}, and trace them to two \emph{different mechanisms}: the concentrated sink to the triangular mask's exposure imbalance, the no-op sink to content-driven bounded-memory token usage (Sec.~\ref{sec:sink}--\ref{sec:hyp}). We show by finetuning that the positional sink is \emph{learned and removable} --- a windowed mask drives it to zero --- while the no-op sink remains and is leaned on more.
