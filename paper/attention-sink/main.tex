...
\caption{Four continued-pretraining regimes, shown as attention masks (each query row attends its shaded key columns). \textbf{Top:} the standard \emph{causal} mask (full, growing history) and a \emph{sliding window} of width $W$ over real recent history. \textbf{Bottom, our constant-memory variants:} a \emph{uniform window}, where every token is conditioned on exactly $W$ tokens via fixed-length training, so it also runs on a recurrence (early tokens padded, grey); and a \emph{sliding window with trainable tokens}, where an always-on or cold-start trainable prefix (orange) replaces the sink. Older real context must be carried by the weights.}
\label{fig:windowed}
\end{figure}
