...
Is the gain from \emph{windowing}, or merely from more finetuning? We answer with a strict control: two $1.3$B models from the flash-linear-attention suite trained on the \emph{same} $100$B tokens with the \emph{same} skeleton, differing only in the token-mixer --- a softmax Transformer and a gated-linear (GLA) model --- evaluated across the full train-window $\times$ deploy-window grid (Table~\ref{tab:strictmatrix}). The softmax base collapses at tight windows ($10.2\!\to\!17.9$ at deploy-$128$); continued \emph{full-causal} pretraining (triangle-CPT) only partly recovers ($14.0$, the control held fixed); windowed finetuning gives near-flat rows --- the same \emph{train small, deploy anywhere} asymmetry as the hybrid model --- so the recovery is from the window, not the extra training. The gated GLA has no attention window: in its native $O(1)$-state mode it reaches $8.71$ with constant memory, beating every windowed-softmax cell --- but only because it always uses full context. When we instead \emph{bound} its memory (resetting the recurrent state every $W$ tokens, the gated analog of windowing), it does \emph{not} inherit train-small-deploy-anywhere: the inference budget dominates and the training window is nearly irrelevant (gla-windowed-$128$ and -$1024$ differ by $<\!1$ ppl), and bounded gla is worse than the windowed softmax at the same budget ($10.99$ vs.\ $9.63$ at $256$). Windowing is thus a learned \emph{softmax} adaptation, not a generic constant-memory recipe: native gating wins with full context, while the windowed-softmax retrofit wins at a bounded budget.
...
\paragraph{The whole zoo.} Is bounded-budget quality just a property of \emph{some} mixer we should have picked? We evaluate the entire flash-linear-attention $1.3$B-100B suite --- softmax, gla, gsa, delta\_net, retnet, all the same skeleton and data --- under the same uniform block budget (Table~\ref{tab:suite}). Off the shelf, none handles a tight budget well: every mixer sits at $\approx$$20$--$23$ perplexity at a $128$-token budget, separating only near full context (delta\_net best). The windowed-\emph{finetuned} softmax reaches $10.6$ at the same budget --- bounded-budget competence comes from training \emph{for} the budget, not from the token-mixer.

\input{tables/suite_budget.tex}
