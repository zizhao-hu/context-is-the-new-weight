...
\caption{The perturbation pipeline. Intended text passes through one of three channels, the
modeled keyboard operators ($1$--$6$), the modeled voice operators ($7$--$12$), or the
TTS$\to$ASR roundtrip baselines ($13$--$14$), and the transmitted text feeds evaluation and
augmentation. The full operator set is in Tab.~\ref{tab:operators} (App.~\ref{app:operators}).}
\label{fig:pipeline}
\end{figure*}
...
with confusion statistics drawn from \todo{ASR confusion source}. Alongside the twelve modeled operators, two TTS$\to$ASR roundtrip baselines (operators $13$ and $14$) speak the text through a synthesis engine and re-hear it with a recognizer, giving reference voice corruptions from real engine pairs rather than modeled statistics (Fig.~\ref{fig:pipeline}; full set in Tab.~\ref{tab:operators}, App.~\ref{app:operators}). Every modeled operator exposes the same severity dial, the fraction of tokens touched, so the two families can be applied at matched rates. The set supports two uses unchanged: perturbing benchmark inputs for evaluation, and perturbing training data for augmentation.
