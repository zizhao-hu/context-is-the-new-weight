...
#SBATCH --output=/project2/jessetho_1732/zizhaoh/context-is-the-new-weight/logs/reactbase_%j.log
#SBATCH --error=/project2/jessetho_1732/zizhaoh/context-is-the-new-weight/logs/reactbase_%j.log
# ReAct-paper replication baseline (Act / ReAct prompting, frozen Qwen3.5-9B) on WebShop.
set -eo pipefail
...
echo "[react-base] starting webshop server on port $PORT ..."
...
python -u scripts/react_baseline_webshop.py --mode "${MODE:?}" --n-episodes "${N_EP:-100}" \
  --sids-file "${SIDS_FILE:?}" --out "${OUT:?}" --model "${MODEL:-Qwen/Qwen3.5-9B}" $Q $VB
echo "DONE react_base $MODE"
