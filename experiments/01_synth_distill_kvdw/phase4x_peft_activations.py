"""Phase 4x — three-way activation comparison for any PEFT adapter.

For one context with a trained PEFT adapter (lora / prompt / prefix), compute
three perturbation vectors at every post-attention and post-MLP residual-stream
site:
  v_ctx     = resid([C; Q]; base)         - resid(Q; base)   # in-context
  v_dist    = resid(Q; θ_C)               - resid(Q; base)   # context-distilled (full FT)
  v_adapter = resid(Q; base + adapter)    - resid(Q; base)   # PEFT method

Then per layer aggregate the L2 norms and pairwise cosines between these three
vectors. Headline question: is v_adapter more similar to v_ctx than v_dist is?

Saves outputs/01_synth_distill_kvdw/<method>_<context>/peft_activations.json.
"""
...
from src import kvdw, models, peft_adapter, synth, use_cases, validation_queries
...
@torch.no_grad()
def compute_three_way(base, distilled, adapter_model, tok, ctx_name, queries, device):
    accum: dict[int, dict] = {}
    n_ok = 0
    ...
    for q in queries:
        try:
            ...
            cap_adp = kvdw.captures_for(adapter_model, no_ids, no_mask)

            for key in cap_base:
                if key not in cap_ctx or key not in cap_dist or key not in cap_adp:
                    continue
                a = cap_base[key].float()
                v_ctx = (cap_ctx[key].float() - a)
                v_dist = (cap_dist[key].float() - a)
                v_adp = (cap_adp[key].float() - a)

                layer, _ = key
                d = accum.setdefault(layer, {
                    "v_ctx_n": [], "v_dist_n": [], "v_adp_n": [],
                    "cos_ctx_dist": [], "cos_ctx_adp": [], "cos_dist_adp": [],
                    "ctx_minus_adp_n": [], "ctx_minus_dist_n": [],
                })
                d["v_ctx_n"].append(v_ctx.norm(dim=-1).mean().item())
                d["v_dist_n"].append(v_dist.norm(dim=-1).mean().item())
                d["v_adp_n"].append(v_adp.norm(dim=-1).mean().item())
                d["cos_ctx_dist"].append(F.cosine_similarity(v_ctx, v_dist, dim=-1).mean().item())
                d["cos_ctx_adp"].append(F.cosine_similarity(v_ctx, v_adp, dim=-1).mean().item())
                d["cos_dist_adp"].append(F.cosine_similarity(v_dist, v_adp, dim=-1).mean().item())
                d["ctx_minus_adp_n"].append((v_ctx - v_adp).norm(dim=-1).mean().item())
    ...
    for layer in sorted(accum):
        ...
        per_layer.append({
            ...
            "v_adapter_norm": sum(d["v_adp_n"]) / max(1, len(d["v_adp_n"])),
            "cos_ctx_dist": sum(d["cos_ctx_dist"]) / max(1, len(d["cos_ctx_dist"])),
            "cos_ctx_adapter": sum(d["cos_ctx_adp"]) / max(1, len(d["cos_ctx_adp"])),
            "cos_dist_adapter": sum(d["cos_dist_adp"]) / max(1, len(d["cos_dist_adp"])),
            "ctx_minus_adapter_norm": sum(d["ctx_minus_adp_n"]) / max(1, len(d["ctx_minus_adp_n"])),
...
def main():
    ...
    print(f"[phase4x:{args.method}] context={args.context}")
    print(f"[phase4x:{args.method}]   distilled at {distilled_dir}")
    print(f"[phase4x:{args.method}]   adapter at {adapter_dir}")
    ...
    base_for_adapter, _ = models.load(cfg["model"], dtype=dtype, device=args.device)
    adapter_model = peft_adapter.load_adapter(base_for_adapter, adapter_dir)
    base.eval()
    distilled.eval()
    adapter_model.eval()
    ...
    out: dict = {"context": args.context, "method": args.method}
    for split, qs in [("train", train_queries), ("val", val_queries), ("ood", ood_queries)]:
        if not qs:
            continue
        print(f"[phase4x:{args.method}] computing {split} ({len(qs)} queries)")
        out[split] = compute_three_way(base, distilled, adapter_model, tok, base_context, qs, args.device)

    (out_dir / "peft_activations.json").write_text(json.dumps(out, indent=2), encoding="utf-8")
    print(f"[phase4x:{args.method}] wrote {out_dir / 'peft_activations.json'}")
