...
#SBATCH --output=/scratch1/zizhaoh/logs/bpe9_%x_%j.log
#SBATCH --error=/scratch1/zizhaoh/logs/bpe9_%x_%j.log
set -eo pipefail
...
BND=$2
EXTRA="${@:3}"
echo "BPE9_START mask=$MASK boundary=$BND extra=$EXTRA"
python $EXP/toy_all8_bpe.py --mask $MASK --boundary $BND $EXTRA
echo "BPE9_JOBDONE mask=$MASK boundary=$BND"
