...
cd $ROOT
KIND=$1            # cptreg | toyreg
SCHEME=$2          # sliding_history | warmup | persistent
W=$3
EXTRA="${@:4}"
echo "REGIME_START kind=$KIND scheme=$SCHEME W=$W extra=$EXTRA"
if [ "$KIND" = "cptreg" ]; then
  python $EXP/cpt_regimes.py --scheme $SCHEME --window $W $EXTRA
else
  python $EXP/toy_regimes.py --scheme $SCHEME --window $W $EXTRA
fi
echo "REGIME_DONE kind=$KIND scheme=$SCHEME W=$W"
